...
{
    public static void SetLayerIncludeChild(this GameObject gameObject, int newLayer, int excludeLayer = -1,
        int excludeLayer2 = -1)
    {
    ...
    public static void GetChildRecursionByName(this Transform parent, string name, bool fullSearch,
        out List<Transform> result)
    {
        ...
        for (int i = 0; i < parent.childCount; i++)
        {
            GetChildRecursionByName(parent.GetChild(i), name, fullSearch, out List<Transform> listExtends);
            result.AddRange(listExtends);
        }
    }
    ...
    public static bool IsChildRecursionCheck(this Transform parent, GameObject check)
    {
        if (parent.gameObject == check)
            return true;
        
        for (int i = 0; i < parent.childCount; i++)
        {
            if (parent.GetChild(i).gameObject == check)
            {
                return true;
            }
        }
        ...
        for (int i = 0; i < parent.childCount; i++)
        {
            bool isSuccess = IsChildRecursionCheck(parent.GetChild(i), check);
