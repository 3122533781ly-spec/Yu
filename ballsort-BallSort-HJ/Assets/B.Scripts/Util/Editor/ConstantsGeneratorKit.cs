...
namespace Prime31.Editor
{
    ...
    {
        private const string FOLDER_LOCATION = "ProjectSpace/VentMatch/Scripts/AutoGenerated/";
        private const string NAMESPACE = "SoyBean";
        ...
        private const string SORTINGLAYERS_FILE_NAME = "SortingLayers.cs";
        ...
        {
            rebuildConstantsClasses(false);
        }


        public static void rebuildConstantsClasses(bool buildResources = true, bool buildScenes = true,
            bool buildTagsAndLayers = true)
        {
            ...
            {
                File.WriteAllText(folderPath + TAGS_FILE_NAME,
                    getClassContent(TAGS_FILE_NAME.Replace(".cs", string.Empty),
                        UnityEditorInternal.InternalEditorUtility.tags));
                File.WriteAllText(folderPath + LAYERS_FILE_NAME,
                    getLayerClassContent(LAYERS_FILE_NAME.Replace(".cs", string.Empty),
                        UnityEditorInternal.InternalEditorUtility.layers));

                Type internalEditorUtilityType = typeof(InternalEditorUtility);
                PropertyInfo sortingLayersProperty = internalEditorUtilityType.GetProperty("sortingLayerNames",
                    BindingFlags.Static | BindingFlags.NonPublic);
                File.WriteAllText(folderPath + SORTINGLAYERS_FILE_NAME, getSortingLayerClassContent(
                    SORTINGLAYERS_FILE_NAME.Replace(".cs", string.Empty),
                    (string[]) sortingLayersProperty.GetValue(null, new object[0])));

                AssetDatabase.ImportAsset("Assets/" + FOLDER_LOCATION + TAGS_FILE_NAME, ImportAssetOptions.ForceUpdate);
                AssetDatabase.ImportAsset("Assets/" + FOLDER_LOCATION + LAYERS_FILE_NAME,
            ...
            if (buildScenes)
            {
                File.WriteAllText(folderPath + SCENES_FILE_NAME,
                    getClassContent(SCENES_FILE_NAME.Replace(".cs", string.Empty),
                        editorBuildSettingsScenesToNameStrings(EditorBuildSettings.scenes)));
                AssetDatabase.ImportAsset("Assets/" + FOLDER_LOCATION + SCENES_FILE_NAME,
                    ImportAssetOptions.ForceUpdate);
            }

            if (buildResources)
            {
                File.WriteAllText(folderPath + RESOURCE_PATHS_FILE_NAME,
                    getResourcePathsContent(RESOURCE_PATHS_FILE_NAME.Replace(".cs", string.Empty)));
                AssetDatabase.ImportAsset("Assets/" + FOLDER_LOCATION + RESOURCE_PATHS_FILE_NAME,
                    ImportAssetOptions.ForceUpdate);
            }
        ...
        private static string getSortingLayerClassContent(string className, string[] labelsArray)
        {
            ...
            foreach (var label in labelsArray)
                output += "\t\t" + "public const int " + toUpperCaseWithUnderscores(label) + " = " +
                          SortingLayer.NameToID(label) + ";\r\n";
		...
		public static int everythingBut(int layer1, int layer2)
		{
			return ~onlyIncluding(layer1, layer2);
		}";
